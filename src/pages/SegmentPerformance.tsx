...
function SegmentKPIs({ segment, data }: { segment: 'B2G' | 'B2B' | 'B2C'; data: SegmentData }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <KPICard label="Win Rate" value={formatPercent(winRate)} status={winRate >= 50 ? 'green' : 'yellow'} icon={Trophy} autoFitText className="bg-kpi-teal" borderAccent="border-l-kpi-teal-border" />
      ...
      <KPICard label="Avg Deal Size" value={formatIDRFull(avgDealSize)} icon={ShoppingCart} autoFitText className="bg-kpi-purple" borderAccent="border-l-kpi-purple-border" />
      <KPICard label="Conversion Rate" value={formatPercent(conversionRate)} status={conversionRate >= 50 ? 'green' : 'yellow'} icon={TrendingUp} autoFitText className="bg-kpi-rose" borderAccent="border-l-kpi-rose-border" />
      <KPICard label="Gross Profit" value={formatIDRFull(grossProfit)} icon={RefreshCw} autoFitText className="bg-kpi-emerald" borderAccent="border-l-kpi-emerald-border" />
    </div>
  );
}

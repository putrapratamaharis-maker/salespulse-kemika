  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard label="Pipeline Value" value={formatIDRFull(pipelineValue)} icon={DollarSign} autoFitText className="bg-gradient-to-br from-indigo-600 to-indigo-500" tooltip="Total nilai deal aktif Anda (Prospect, Quotation, Negotiation)" />
        <KPICard label="Weighted Forecast" value={formatIDRFull(weightedForecast)} icon={TrendingUp} autoFitText className="bg-gradient-to-br from-teal-600 to-teal-500" tooltip="Σ (value × probability / 100) dari deal aktif Anda, tidak termasuk PO Secured, Invoice Issued, Canceled, Lost" />
        <KPICard label="Active Deals" value={String(activeDeals.length)} icon={GitBranch} autoFitText className="bg-gradient-to-br from-sky-600 to-sky-500" tooltip="Jumlah deal Anda yang masih dalam tahap aktif (belum won/lost/canceled)" />
        <KPICard label="Avg Probability" value={formatPercent(avgProbability)} status={avgProbability >= 50 ? 'green' : 'yellow'} icon={TrendingUp} autoFitText className="bg-gradient-to-br from-amber-500 to-amber-400" tooltip="Rata-rata probability dari seluruh deal aktif Anda" />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <KPICard label="Total Ticket/Card" value={String(deals.length)} icon={GitBranch} autoFitText className="bg-gradient-to-br from-sky-600 to-sky-500" tooltip="Total kartu/tiket deal di semua stages (termasuk PO Secured, Invoice Issued, Canceled, Lost)" />
        <KPICard label="Next Month Closing" value={String(nextMonthClose.length)} changeLabel={nextMonthClose.length > 0 ? formatIDRFull(nextMonthClose.reduce((s, d) => s + d.value, 0)) : 'No deals'} icon={CalendarClock} status={nextMonthClose.length > 0 ? 'green' : 'yellow'} autoFitText className="bg-gradient-to-br from-emerald-600 to-emerald-500" tooltip="Deal yang expected close date-nya dalam bulan depan" />

  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <KPICard label="MY ACTUAL REVENUE YTD" value={formatIDRFull(revenueYTD)} icon={Banknote} status={achievementPctYTD >= 100 ? 'green' : achievementPctYTD >= 80 ? 'yellow' : 'red'} autoFitText className="bg-gradient-to-br from-indigo-600 to-indigo-500" tooltip="Total nilai deal Anda pada tahap PO Secured DAN Invoice Issued di tahun berjalan, berdasarkan PO/Won/Closed Date" />
        <KPICard label={`MY TOTAL REVENUE TARGET ${selectedYear}`} value={formatIDRFull(totalTargetYear)} icon={Target} autoFitText className="bg-gradient-to-br from-amber-500 to-amber-400" tooltip={`Jumlah seluruh revenue_target Anda di tahun ${selectedYear} dari Admin Panel Sales Targets (semua bulan & segmen)`} />
        <KPICard label={`MY TARGET ACHIEVEMENT ${selectedYear}`} value={formatPercent(achievementPctYTD)} status={getAchievementStatus(achievementPctYTD)} icon={Target} autoFitText className="bg-gradient-to-br from-sky-600 to-sky-500" tooltip={`Revenue YTD ÷ Total Revenue Target ${selectedYear} × 100%`} />
      </div>

      {/* Row 2: MTD */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <KPICard label="MY ACTUAL REVENUE MTD" value={formatIDRFull(revenueMTD)} change={lastMonthChange} changeLabel="vs last month" icon={DollarSign} autoFitText className="bg-gradient-to-br from-teal-600 to-teal-500" tooltip="Total nilai deal Anda pada tahap PO Secured DAN Invoice Issued di bulan berjalan, berdasarkan PO/Won/Closed Date" />
        <KPICard label={`MY REVENUE TARGET ${monthName}`} value={formatIDRFull(targetRevenue)} icon={Target} autoFitText className="bg-gradient-to-br from-amber-500 to-amber-400" tooltip={`Revenue target Anda untuk bulan ${monthName} dari tabel targets`} />
        <KPICard label={`MY TARGET ACHIEVEMENT ${monthName}`} value={formatPercent(achievementPct)} status={getAchievementStatus(achievementPct)} icon={Target} autoFitText className="bg-gradient-to-br from-sky-600 to-sky-500" tooltip={`Revenue MTD ÷ Revenue Target × 100% untuk bulan ${monthName}`} />
      </div>

      {/* Row 3: Margin & AR */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard label="MY GROSS MARGIN" value={formatPercent(marginPct)} status={marginPct >= MARGIN_THRESHOLD ? 'green' : 'red'} icon={Percent} autoFitText className="bg-gradient-to-br from-emerald-600 to-emerald-500" tooltip="Gross Profit ÷ Net Sales × 100% dari invoice Anda. Threshold hijau ≥ 17%" />
        <KPICard label="MY GP CONTRIBUTION" value={formatIDRFull(grossProfitMTD)} icon={TrendingUp} autoFitText className="bg-gradient-to-br from-cyan-600 to-cyan-500" tooltip="Total gross_profit dari invoice Anda di bulan berjalan, berdasarkan PO/Won/Closed Date" />
        <KPICard label="MY CASH-IN (PAID)" value={formatIDRFull(cashInValue)} icon={Banknote} status={cashInValue > 0 ? 'green' : 'yellow'} changeLabel={`${paidInvoices.length} invoices`} autoFitText className="bg-gradient-to-br from-emerald-600 to-emerald-500" tooltip="Total net_sales dari invoice Anda yang sudah dibayar (memiliki paid_date)" />
        <KPICard label="MY OUTSTANDING AR" value={formatIDRFull(outstandingAR)} icon={CreditCard} status={outstandingAR > 0 ? 'red' : 'green'} changeLabel={`${inv.filter(i => !i.paid_date).length} unpaid`} autoFitText className="bg-gradient-to-br from-rose-500 to-rose-400" tooltip="Total net_sales dari invoice Anda yang belum dibayar (paid_date kosong)" />
      </div>

...
BEGIN
  ...
  mtd_won AS (
    ...
    WHERE EXTRACT(MONTH FROM expected_close_date::date) = _current_month
      AND EXTRACT(YEAR FROM expected_close_date::date) = _current_year
  ),
  ytd_won AS (
    SELECT * FROM won_deals
    WHERE EXTRACT(YEAR FROM expected_close_date::date) = _current_year
  ),
  all_invoices AS (
  ...
  SELECT jsonb_build_object(
    ...
    'monthly_trend', (
      ...
      FROM (
        SELECT EXTRACT(MONTH FROM expected_close_date::date)::int AS mn, segment AS seg, SUM(value) AS rev
        FROM ytd_won
        GROUP BY mn, seg
        ORDER BY mn, seg
      ) mt

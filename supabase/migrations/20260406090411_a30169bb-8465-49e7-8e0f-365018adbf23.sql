...
-- Add revenue_date column to deals
ALTER TABLE public.deals ADD COLUMN revenue_date date;

-- Backfill: for won/invoiced deals, set revenue_date = expected_close_date
UPDATE public.deals
SET revenue_date = expected_close_date
WHERE stage IN ('po_secured', 'invoice_issued');

-- Update RPC function to use revenue_date
DROP FUNCTION IF EXISTS public.get_executive_summary_kpis(integer, integer);

CREATE FUNCTION public.get_executive_summary_kpis(_current_month integer, _current_year integer)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  WITH
  won_deals AS (
    SELECT value, revenue_date, segment, account_id
    FROM public.deals
    WHERE stage IN ('po_secured', 'invoice_issued')
  ...
  mtd_won AS (
    ...
    WHERE EXTRACT(MONTH FROM revenue_date) = _current_month
      AND EXTRACT(YEAR FROM revenue_date) = _current_year
  ),
  ytd_won AS (
    SELECT * FROM won_deals
    WHERE EXTRACT(YEAR FROM revenue_date) = _current_year
  ),
  all_invoices AS (
  ...
  kpi_agg AS (
    SELECT
      ...
      (SELECT COALESCE(SUM(revenue_target), 0) FROM all_targets) AS total_target,
  ...
  SELECT jsonb_build_object(
    ...
    'monthly_trend', (
      ...
      FROM (
        SELECT EXTRACT(MONTH FROM revenue_date)::int AS mn, segment AS seg, SUM(value) AS rev
        FROM ytd_won
        GROUP BY mn, seg
        ORDER BY mn, seg
      ) mt

-- Add role guard to get_executive_summary_kpis to prevent viewers from seeing corporate financials
...
DECLARE
  result jsonb;
BEGIN
  -- Restrict to admins/super_admins only — corporate-wide financial KPIs
...
-- Add role guard to get_accounts_basic — prevents enumeration of all customer names by viewers.
-- Sales-facing flows already use get_accounts_with_pic_for_user which has owner/supervisor scoping.
CREATE OR REPLACE FUNCTION public.get_accounts_basic()
 RETURNS TABLE(id uuid, name text, segment text, type text, region text, city text, status text, customer_id text, sales_id uuid)
 LANGUAGE plpgsql

...
BEGIN
  WITH
  won_deals AS (
    SELECT value, updated_at, segment, account_id
    FROM public.deals
    WHERE stage IN ('po_secured', 'invoice_issued')
  ),
  mtd_won AS (
    SELECT * FROM won_deals
    WHERE EXTRACT(MONTH FROM updated_at::date) = _current_month
      AND EXTRACT(YEAR FROM updated_at::date) = _current_year
  ),
  ytd_won AS (
    SELECT * FROM won_deals
    WHERE EXTRACT(YEAR FROM updated_at::date) = _current_year
  ),
  all_invoices AS (
  ...
  SELECT jsonb_build_object(
    ...
    'monthly_trend', (
      ...
      FROM (
        SELECT EXTRACT(MONTH FROM updated_at::date)::int AS mn, segment AS seg, SUM(value) AS rev
        FROM ytd_won
        GROUP BY mn, seg
        ORDER BY mn, seg
      ) mt

-- Allow all authenticated users to read corporate-wide aggregate KPIs
-- These RPCs return read-only summary data shown on shared dashboards.
-- Row-level data still respects RLS via direct table queries elsewhere.
...
CREATE OR REPLACE FUNCTION public.get_segment_deals()
 ...
 SET search_path TO 'public'
AS $function$
  SELECT d.value, d.stage, d.segment FROM public.deals d;
$function$;
...
CREATE OR REPLACE FUNCTION public.get_segment_targets()
 ...
 SET search_path TO 'public'
AS $function$
  SELECT t.revenue_target::numeric, t.segment, t.month FROM public.targets t;
$function$;

...
BEGIN
  ...
  kpi_agg AS (
    SELECT
      (SELECT COALESCE(SUM(net_sales), 0) FROM mtd_inv) AS revenue_mtd,
      (SELECT COALESCE(SUM(gross_profit), 0) FROM mtd_inv) AS gp_mtd,
      (SELECT COALESCE(SUM(net_sales), 0) FROM ytd_inv) AS revenue_ytd,
  ...
  SELECT jsonb_build_object(
    ...
    'segment_revenue', (
      ...
      FROM (
        SELECT segment AS seg, COALESCE(SUM(net_sales), 0) AS rev
        FROM all_invoices
        GROUP BY segment
      ) sr
    ),
    'customer_revenue', (
      ...
      FROM (
        SELECT account_id AS aid, SUM(net_sales) AS rev
        FROM all_invoices
        GROUP BY account_id
        ORDER BY rev DESC
        LIMIT 10
    ...
    'region_revenue', (
      ...
      FROM (
        SELECT COALESCE(a.region, 'Unknown') AS r, SUM(i.net_sales) AS rev
        FROM all_invoices i
    ...
    'monthly_trend', (
      ...
      FROM (
        SELECT EXTRACT(MONTH FROM issue_date::date)::int AS mn, segment AS seg, SUM(net_sales) AS rev
        FROM ytd_inv
        GROUP BY mn, seg
        ORDER BY mn, seg
      ) mt
